...
export function DashboardWorkspace() {
  ...
  return (
    <section className="page dashboard-page">
      ...
      <div className="metric-strip"><div><span className="metric-icon metric-purple"><Clock3 size={18} /></span><span><small>Conversation time</small><strong>{formatDuration(totalSeconds)}</strong><em>Across {library.items.length} meetings</em></span></div><div><span className="metric-icon metric-green"><ListChecks size={18} /></span><span><small>Open action items</small><strong>{openActions.length}</strong><em>{actions.length} captured in total</em></span></div><div><span className="metric-icon metric-gold"><Users size={18} /></span><span><small>People in meetings</small><strong>{people.size}</strong><em>Active conversation network</em></span></div><div><span className="metric-icon metric-blue"><CheckCircle2 size={18} /></span><span><small>Task completion</small><strong>{actions.length ? Math.round(((actions.length - openActions.length) / actions.length) * 100) : 0}%</strong><em>{actions.length - openActions.length} follow-ups closed</em></span></div></div>
