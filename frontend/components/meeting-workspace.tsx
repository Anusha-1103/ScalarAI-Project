  ...
  return (
    <div className="meeting-detail-page">
      <header className="meeting-detail-header">
        ...
        <div className="meeting-title-block"><h1>{meeting.title}</h1><div className="meeting-meta"><span><CalendarDays size={14} />{formatMeetingDate(meeting.meetingAtUtc)}</span><span><Clock3 size={14} />{formatTimestamp(meeting.durationInSeconds)}</span><span><Users size={14} />{meeting.participants.length} attendees</span></div></div>
      ...
      <div className="meeting-detail-grid">
        <section className={`insights-panel ${mobilePane !== "notes" ? "mobile-pane-hidden" : ""}`} aria-label="Meeting insights">
          ...
          <div className="insights-scroll">
            {tab === "summary" && <div className="summary-content"><div className="section-heading"><span className="summary-icon"><Sparkles size={17} /></span><div><p className="eyebrow">AI meeting notes</p><h2>Conversation summary</h2></div></div><p className="overview">{meeting.summary?.overview ?? "No summary is available for this meeting."}</p><h3>Key points</h3><ul className="key-points">{meeting.summary?.keyPoints.map((point) => <li key={point}><span />{point}</li>)}</ul></div>}
