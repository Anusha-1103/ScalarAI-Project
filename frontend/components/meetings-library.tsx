...
import { ArrowDownUp, AudioWaveform, CalendarDays, CheckCircle2, ChevronRight, Clock3, FileAudio2, Filter, ListChecks, Plus, RefreshCw, Search, Tag, Users } from "lucide-react";
...
export function MeetingsLibrary() {
  ...
  const params = useMemo(() => {
    ...
    return value;
  }, [dateFrom, dateTo, deferredSearch, participant, sortOrder, tag]);
  const meetings = useQuery({ queryKey: ["meetings", params.toString()], queryFn: () => getMeetings(params) });
  const hasActiveFilters = Boolean(search.trim() || participant.trim() || tag.trim() || dateFrom || dateTo);
  ...
  return (
    <section className="page meetings-page">
      ...
      <div className="library-toolbar">
        ...
        <button className={`icon-button filter-more ${filtersOpen || tag || dateFrom || dateTo ? "filter-active" : ""}`} title="More filters" aria-label="More filters" aria-expanded={filtersOpen} onClick={() => setFiltersOpen((value) => !value)}><Filter size={17} /></button>
      </div>

      {filtersOpen && <div className="date-filter-panel"><label>Tag<span className="compact-filter"><Tag size={14} /><input type="text" value={tag} onChange={(event) => setTag(event.target.value)} placeholder="Product" aria-label="Filter by tag" /></span></label><label>From<input type="date" value={dateFrom} onChange={(event) => setDateFrom(event.target.value)} /></label><label>To<input type="date" min={dateFrom || undefined} value={dateTo} onChange={(event) => setDateTo(event.target.value)} /></label><button className="button button-secondary" disabled={!tag && !dateFrom && !dateTo} onClick={() => { setTag(""); setDateFrom(""); setDateTo(""); }}>Clear filters</button></div>}
      ...
      <div className="meeting-list" aria-live="polite">
        ...
        {meetings.data?.items.length === 0 && (
          hasActiveFilters
            ? <div className="state-panel"><Search size={24} /><h2>No matching meetings</h2><p>Try a different title, person, tag, or date range.</p><button className="button button-secondary" onClick={() => { setSearch(""); setParticipant(""); setTag(""); setDateFrom(""); setDateTo(""); }}>Clear filters</button></div>
        ...
        {meetings.data?.items.map((meeting) => (
          <Link href={`/meetings/${meeting.id}`} className="meeting-row" key={meeting.id}>
            ...
            <div className="meeting-main"><div className="meeting-title-row"><h2>{meeting.title}</h2><span className="source-pill"><AudioWaveform size={11} />{sourceLabel(meeting.sourceType)}</span></div><p>{meeting.summaryPreview ?? "Transcript ready to review"}</p><span className="meeting-tags">{meeting.tags.map((item) => <span key={item}><Tag size={10} />{item}</span>)}</span><span className="meeting-mobile-meta"><Clock3 size={12} />{formatDuration(meeting.durationInSeconds)} · {meeting.participants.length} people</span></div>
